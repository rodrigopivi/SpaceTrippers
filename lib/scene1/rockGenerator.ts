module Core.Scene1 {
    ...
    export class RockGenerator {
        ...
        constructor(scene: Scene, targetObject: BABYLON.Mesh) {
            ...
            BABYLON.SceneLoader.ImportMesh("", "/assets/", "rockcandy.babylon", this.scene.scene,(newMeshes) => {
            ...
            this.recursiveRocksCreation = () => {
                if (this.currentRocks.length < this.numberOfRocks) {
                    setTimeout(() => { this.addRock(); this.recursiveRocksCreation(); }, Core.getRandomInRange(2000, 4000));
                }
            };
            ...
            this.addRock = (): void => {
                if (this.originalRock) {
                    ...
                    var laneIndex = Core.getRandomInRange(0, this.scene.track.rows.lanesPositionX.length - 1);
                    var newRock: IRock = <IRock>{
                        ...
                        rotation: new BABYLON.Vector3(
                            Core.getRandomInRange(-1, 1) * 0.01,
                            Core.getRandomInRange(-1, 1) * 0.01,
                            Core.getRandomInRange(-1, 1) * 0.01
                            ),
                        isTransparent: false,
